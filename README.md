...
python3 {the path of this directory}/fcs.py screen genome --fasta {genome fasta} --out-dir {output directory} --gx-db {local gxdb database} --tax-id {tax id}
...
#Clean genome
cat {original genome fasta} | python3 {the path of this directory}/fcs.py clean genome --action-report ./{output directory}/{genome fasta basename}.{tax id}.fcs_gx_report_modified.txt --output {output clean directory}/{genome fasta basename}_clean.fasta --contam-fasta-out {output clean directory}/contam.fasta
```
Note
...
nohup bash -c "time ({the path of this directory}/trf_mask.sh -i {genome after repeat masking} -o 04_TRF_mask -t 128 > run_trf_mask.log 2>&1)" 2> run_trf_time.log > /dev/null &
```
...
nohmmonly --tblout {output file path}.tblout --fmt 2 --clanin {Rfam db}/Rfam.clanin --oskip {Rfam db}/Rfam.cm {genome fasta} > {stderr output log}.cmscan 2> run_cmscan.log" > run_cmscan_time.log 2>&1 &

nohup bash -c "time tRNAscan-SE -E -I --gff 06_ncRNA/{outfile name}_tRNAscan.gff --stats 06_ncRNA/{outfile name}.stats -d --thread 128 00_annotation_start_genome/{nuclear ummasked genome}.fasta > run_{outfile name}_tRNA.log 2>&1" > run_{outfile name}_tRNAscan_time.log 2>&1 &
```
...
nohup ../Annotation-Toolkit/run_cutadapt.sh -d {raw RNA-seq dir} -o {cleaned RNA-seq dir} -t 128 -l 5 -q 20 -Q 20 -f sample_trim.txt > run_cutadapt.log 2>&1 &

#Run RNA mapping with STAR and keep only reads flagged as 2.
nohup python ../Annotation-Toolkit/run_rna_mapping_star_2pass.py --genomepath {genome} --genomedir {genome index dir} --wd {dir for cleaned.fq/fastq} --out_dir {outdir} --threads 128 > run_rna_mapping.log 2>&1 &
```
 
## Run Braker3
```
nohup bash -c "time (../Annotation-Toolkit/run_braker3.sh -t 120 -g {TRF masked genome} -p {protein evidence} -s {parameter set name} -w {outdir} -b {bam file dir} -l {BUSCO odb}  > run_braker3_v3.log 2>&1)" 2> run_braker3_v3_time.log > /dev/null &
```
...
nohup python ../Annotation-Toolkit/eva_annotation.py structural -g 07_merge_gff/merged_fix.gff -f {genome fasta} -b {file basename} -o 07_merge_gff/ -p 07_renamed_braker3/braker_rename
...
#mitohifi/mitoz
python ../Annotation-Toolkit/fix_merge_sort_gff.py -I infernal 06_ncRNA/marine_tilapia_MTW02_ncRNA.tblout -I
...
nohup python ../Annotation-Toolkit/eva_annotation.py structural -g 07_merge_gff/merged_fix.gff -f {genome fasta} -b {file basename} -o 07_merge_gff/ --split_scaffolds {the scaffold desired to be ccalculated separately, ex. mitochondria} > run_eva_combined.log 2>&1 &
...
#mitohifi/mitoz
python ../Annotation-Toolkit/molas_fasta-intron-lowercase.py -g 00_annotation_start_genome/MTW_genome_clean_nuclear_mito.fasta -r 07_merge_gff/merged_fix.gff -p M_tai -a combined -v --nmitopep --table 5 --pep_braker 06_braker3_v3/braker.aa --nameprefix M_tai -d 08_asian_hard_clam_MOLAS_input
```
